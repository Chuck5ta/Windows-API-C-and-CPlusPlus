...
DWORD dwBaseAddressOfExe = NULL;

bool IsGameAvail = false;
bool bExitBTNClicked = false;
bool bHealthBTnStateIsOn = false;
bool bMoneyBTnStateIsOn = false;
bool bManaBTnStateIsOn = false;
bool bSkullsBTnStateIsOn = false;
bool bGameConnected = false;

// Health related variables
bool HealthStatus;
FLOAT HealthValue = 0xC8; // 100 dec
DWORD HealthBaseAddress = { 0x7A4454 };  // 0xB92450
DWORD HealthOffsets[] = { 0x20, 0x18, 0x54 }; // 3 level pointer

											  // Money related variables
BYTE MoneyValue[] = { 0x88, 0x13, 0x0, 0x0 }; // 5000 money
DWORD MoneyBaseAddress = { 0x7A4454 }; 
DWORD MoneyOffsets[] = { 0x20, 0x18, 0x7C }; // 3 level pointer

											 // Mana related variables
FLOAT ManaValue = 0x64; // 100 dec
DWORD ManaBaseAddress = { 0x7A4454 };
DWORD ManaOffsets[] = { 0x20, 0x18, 0x5C }; // 3 level pointer

											// Skulls related variables
BYTE SkullsValue[] = { 0x20, 0x03, 0x0, 0x0 }; // 800 skulls
DWORD SkullsBaseAddress = { 0x7B830C };  // NOT WORKING!!!
// DWORD SkullsOffsets[] = { 0x0 }; // no offsets
...
DWORD FindDmaAddy(int PointerLevel, HANDLE hProcHandle, DWORD Offsets[], DWORD dwBaseAddressOfHack)
{
	//DECLARE BASE ADDRESS
	DWORD pointer = dwBaseAddressOfHack + dwBaseAddressOfExe;             // Declare a pointer of DWORD
...
void WriteToMemory(HANDLE hProcHandle)
{
	...
	if (bSkullsBTnStateIsOn) // NOT WORKING!!!
	{
...
DWORD GetBaseAddress(HWND hwnd)
{
	...
	SendMessage(hBtnGameConnectionState, WM_SETTEXT, 0, (LPARAM)"Failed to connect!");
	return NULL;
}
	...
	if (Process32First(hProcSnap, &pe32))
	{
		do
		{
			if (strcmp(pe32.szExeFile, ProcessName) == 0)
			{
				...
				{
					// failed
					SendMessage(hBtnGameConnectionState, WM_SETTEXT, 0, (LPARAM)"ERROR: Cannot connect to game!");
					return false;
				}
				else
				{
					...
					dwBaseAddressOfExe = GetBaseAddress(hwnd);
					if (dwBaseAddressOfExe != NULL)
					{
						SendMessage(hBtnGameConnectionState, WM_SETTEXT, 0, (LPARAM)"Acquired base address of exe");
		//				SendMessage(hBtnGameConnectionState, WM_SETTEXT, 0, (LPARAM)ConvertDWORDToString(dwBaseAddressOfExe).c_str());
					}
					else
						SendMessage(hBtnGameConnectionState, WM_SETTEXT, 0, (LPARAM)"Failed to acquire base address!");
...
void applyHacks(HWND hwnd)
{
	...
	if (IsGameAvail)
	{
		if (bHealthBTnStateIsOn || bMoneyBTnStateIsOn || bManaBTnStateIsOn || bSkullsBTnStateIsOn)
		{
	...
	switch (msg)
	{
		...
		case WM_COMMAND:   // processes Menu commands and window events
			...
			case ID_HEALTH_BUTTON: // switch the Money button between On and Off
			{
				...
				bHealthBTnStateIsOn = !bHealthBTnStateIsOn;
			...
			{
				// set the state of the button
				bMoneyBTnStateIsOn = !bMoneyBTnStateIsOn;
			...
			{
				// set the state of the button
				bManaBTnStateIsOn = !bManaBTnStateIsOn;
			...
			{
				// set the state of the button
				bSkullsBTnStateIsOn = !bSkullsBTnStateIsOn;

				if (bSkullsBTnStateIsOn)
				{
